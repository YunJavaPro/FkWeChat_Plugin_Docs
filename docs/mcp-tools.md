...
当大模型客户端（如 Cursor, Claude Desktop 等）连接到 MCP 服务端时，便可以自主识别并调用您编写的脚本。

::: warning 使用前提
在脚本中注册的 MCP 工具要能够被大模型正常识别和调用，**您必须在微信中开启模块自带的 MCP 服务功能**。
...
* **方法签名**: 
  * `void addMcpTool(String methodName, String description)`
  * `void addMcpTool(String toolName, String methodName, String description)`
* **参数说明**:
...
onLoad() {
    ...
    addMcpTool("send_wechat_msg", "通过我当前的微信向指定好友的 wxid 或群聊 ID 发送文本消息");
}
```
...
onLoad() {
    addMcpTool("query_friend_remark", "根据传入的微信号(wxid)查询该好友在当前微信中的备注名字");
}
```
...
onLoad() {
    addMcpTool("get_today_msg_count", "获取并统计指定的群聊或单聊中，今天零点至今产生的消息总数");
}
```