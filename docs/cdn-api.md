...
FkWeChat 提供了独立的 `CdnApi` 接口供脚本调用。它与微信底层的 C++ mars cdn 引擎深度连通，支持多线程分片传输、断点续传以及**自动 AES 密钥解密**。
...
* **示例**:
  ```java
  // 在消息监听回调中，实现静默自动下载所有收到的图片和视频
  onMsg(msg) {
      if (msg.isImage() || msg.isVideo()) {
          downMsgMedia(msg, new CdnApi.CdnCallback() {
